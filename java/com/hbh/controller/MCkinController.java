...
@Controller
@RequestMapping("/manage/flatform/ckin")
public class MCkinController {
	...
	@RequestMapping("/ruku")
	public String ruku(String inid, HttpServletRequest request, Model model) {
		Ckin ckin = ckinServiceImp.getbyid(inid);
		if (ckin != null && ckin.getStatus() != 1) {
			// 库存增加
			kcxxServiceImp.addStock(ckin.getProid(), ckin.getNum(), ckin.getPname());
			// 修改进货单状态为已入库
			ckin.setStatus(1);
